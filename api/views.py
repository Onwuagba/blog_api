...
def log_error(message):
    logging.error("Error retrieving object: %s", message)
...
class PostDetailView(APIView):
    ...
    def get(self, request, **kwargs):
        ...
        try:
            if queryset := self.get_object(obj_id):
    ...
    def patch(self, request, **kwargs):
        ...
        try:
            news_source = "API"
            news_obj = self.get_object(obj_id, news_source)
            serializer = self.serializer_class(
                news_obj, data=request.data, partial=True
            )
            if serializer.is_valid(raise_exception=True):
                serializer.save()
...
class CommentDetailView(ListAPIView):
    ...
    def get(self, request, **kwargs):
        ...
        try:
            queryset = self.filter_queryset(self.get_queryset(obj_id))
            if page := self.paginate_queryset(queryset):
                serializer = self.serializer_class(page, many=True)
                query_response = self.get_paginated_response(serializer.data)
                res = query_response.data
